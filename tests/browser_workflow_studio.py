...
async def main():
    ...
    async with async_playwright() as p:
        ...
        await studio.get_by_role('button',name='测试轨迹',exact=True).click()
        await studio.locator('[data-test=model]').fill('gemini 3.1 Pro')
        await studio.locator('[data-test=captures]').fill('{"current":"Pro"}')
        await studio.get_by_role('button',name='模拟分支路径',exact=True).click()
        await expect(studio.locator('.trace')).to_contain_text('else',timeout=30000)
