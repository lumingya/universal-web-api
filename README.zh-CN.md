...
| **模型名称** | 可任意填写；也可以直接填 `qwen`、`gemini`、`chat.qwen.ai`、`gemini.google.com` 这类值来自动匹配对应站点标签页 |
...
> 默认路由补充：如果请求体里的 `model` 填的是当前已打开站点的别名或域名，例如 `qwen`、`gemini`、`chat.qwen.ai`、`gemini.google.com`，系统会自动按对应网站分配标签页，而不是走普通空闲页随机分配。
...
现在这块还额外带了 **内部修复重试**。如果模型第一次吐出来的 `tool_calls` 结构不合法，后端会按你在 **控制面板 → 设置 → 环境配置 → 函数调用** 里配置的策略，自动再给模型一次修正机会。
...
- `TOOL_CALLING_RETRY_STRATEGY`：重试策略。`聚焦修复` 只发送必要修复信息；`完整上下文` 会把原对话和修复反馈一起发回去。
- `TOOL_CALLING_INTERNAL_RETRY_MAX`：内部修复重试次数。设成 `0` 就是关闭自动修复。
- `TOOL_CALLING_MAX_TOOL_RESULT_CHARS`：单条 Tool Result 最大字符数，避免超大结果把网页模型直接撑爆。
...
> 💡 你可以自定义更多选择器（如「临时对话按钮」），然后在工作流里添加对应的点击步骤。在控制面板的选择器配置项中点击「测试」按钮，现在会进入升级后的选择器测试工作台：不仅能看命中数量，还能看命中详情、推荐候选和风险提示。

如果你是第一次配站点，推荐顺序是：

1. 先配 `input_box`
2. 再配 `send_btn`
3. 再配 `result_container`
4. 每填一个字段就立刻点一次测试
5. 在本地 `selector-practice.html` 工作台里切换场景、刷新随机 class，确认你的写法不是靠动态类名碰运气
...
| `JS_EXEC` | 在当前页面上下文执行 JavaScript，适合处理抽象动作或页面内脚本切换 |
| `STREAM_WAIT` | 等待响应完成 |

现在可视化工作流编辑器也已经能保留这些抽象动作了。`WAIT`、`KEY_PRESS`、`JS_EXEC` 不会再被默默吞掉；它们会作为独立步骤保留，保存回 JSON 时顺序也会跟可视化里的小球顺序一致。
...
#### 给非流式站点配置网络拦截器（适合直接交给 AI）

这里的“非流式”指 **目标网站自己的响应不是 SSE 增量流，而是一次性返回完整 JSON / 文本**。  
对本项目来说，它依然走 `stream_config.mode=network` 这一套监听；区别只在于解析器通常第一次拿到完整响应后就应该返回 `done=True`。如果你最终对外调用的是 `stream=false`，也**不需要再写第二套逻辑**，项目内部仍然会先走这套网络监听，再把结果拼成最终的非流式响应。

项目里已经附带同内容文件：[static/拦截请求发生器.txt](./static/拦截请求发生器.txt)。如果你是从内置教程页进入，已经可以直接在教程里展开查看，不用再回桌面找原文件。

**最省事的做法：**

1. 在脚本控制的浏览器里打开目标站点，并停在真正发消息的页面。
2. 打开开发者工具的 Console。
3. 优先使用项目内置的 `static/拦截请求发生器.txt`，把内容全部复制后粘贴进去并回车；如果你习惯用桌面原文件，也可以继续用桌面的同名文件。
4. 手动在网页上发送一条最简单的问题，让站点真实请求完整跑一遍。
5. 请求结束后，在 Console 里执行 `exportRequests()`，浏览器会下载一个请求导出文件。
6. 把这个导出文件发给 AI，并同时附上 `app/core/parsers` 里一两个最接近的现有解析器、`app/core/parsers/base.py`、`app/core/parsers/__init__.py`，让 AI 直接改代码。
7. 让 AI 新建 `app/core/parsers/xxx_parser.py`，并同步修改 `app/core/parsers/__init__.py` 完成导入与注册。
8. 回到控制台，给对应站点/预设打开网络拦截模式，填好 `listen_pattern`，解析器选择你刚创建的那个。
9. 用真实请求开始测试；如果提取结果不对，把“导出的请求文件 + 当前错误结果 + 你新建的 parser 文件”继续发给 AI 迭代。

**发给 AI 时最好明确这 4 点：**

- 这是“非流式网站响应”，优先从完整响应体里直接提取最终文本。
- 解析器要继承 `ResponseParser`，返回格式保持 `content / images / done / error`。
- 非流式场景第一次成功提取后通常就应该 `done=True`。
- 同一份完整 body 重复进入时不要重复追加内容，避免重复输出。

可以直接把下面这段提示词一起发给 AI：

```text
这是一个新站点的非流式网络请求导出文件。请参考 app/core/parsers 目录里现有解析器，帮我新增一个解析器。

要求：
1. 在 app/core/parsers/xxx_parser.py 新建解析器类，继承 ResponseParser
2. 这个站点是非流式响应，请从完整响应体里提取最终文本
3. 第一次成功解析后应返回 done=True
4. 如果同一份完整 body 重复进入，不要重复输出内容
5. 如有图片，填到 images
6. 同步修改 app/core/parsers/__init__.py，完成导入和注册
7. 告诉我站点配置里应该填什么 listen_pattern 和 parser
...
- `listen_pattern` 先填最稳定的请求路径关键词，不要一上来写太宽。
- 如果导出的请求太多，可以先用 `viewRequests()`、`searchRequests('chat')`，或者 `exportRequests({ minPriority: 'MEDIUM' })` 缩小范围。
- 真正容易错的通常不是“注册解析器”这一步，而是“拿错请求”或“给 AI 的参考样本不够”。
