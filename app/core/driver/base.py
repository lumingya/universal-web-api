...
class TabDriver(Protocol):
    ...
    def find(self, locator: str, timeout: Optional[float] = None) -> Optional[ElementHandle]:
        """timeout 为 None 时沿用驱动的默认等待。"""

    def find_all(self, locator: str, timeout: Optional[float] = None) -> List[ElementHandle]: ...
