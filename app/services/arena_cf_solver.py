"""
app/services/arena_cf_solver.py - Cloudflare Turnstile / 5s 盾高优先级自动过盾服务

职责：
1. 探测并等待可点击人机验证按钮出现（若 DOM 未解析到精确元素则使用固定/自适应保底坐标）；
2. 复用项目的低熵模式移动轨迹（smooth_move_mouse + cdp_precise_click）平滑移动并精确点击；
3. 确认验证通过（最多尝试 3 次，连续 3 次失败直接抛出错误）；
4. 通过后比对当前 URL 与本次请求实际占用的目标 URL：
   - 若不一致（如过盾后页面被重置到首页），自动跳转回实际占用 URL；
   - 若一致，则直接结束并恢复工作流。
"""
...
def _get_occupied_url(tab, session=None) -> str:
...
def _wait_for_clickable_point(
    ...
    while time.time() < deadline:
        if raise_if_cancelled:
            raise_if_cancelled()
        try:
            probe = tab.run_js(_FIND_CLICK_POINT_JS)
...
def _wait_for_challenge_resolved(
    ...
    while time.time() < deadline:
        if raise_if_cancelled:
            raise_if_cancelled()
        try:
            probe = tab.run_js(_PAGE_READY_PROBE_JS)
...
def solve_arena_turnstile_challenge(
    ...
    occupied_url = _get_occupied_url(tab, session)
    log.info(f"[CF_SOLVER] 启动 Arena 5s 盾自动处理: 实际占用目标 URL={occupied_url}")
    ...
    while attempts < max_attempts:
        ...
        smooth_move_mouse(tab, (start_x, start_y), (click_x, click_y))
        _interruptible_sleep(random.uniform(0.06, 0.14), raise_if_cancelled=raise_if_cancelled)
        click_ok = cdp_precise_click(tab, click_x, click_y)
        if not click_ok:
    ...
    if not url_matches and occupied_url and _is_valid_arena_url(occupied_url):
        log.warning(
